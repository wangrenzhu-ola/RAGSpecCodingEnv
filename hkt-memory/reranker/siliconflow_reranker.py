...
class SiliconFlowReranker(CrossEncoderReranker):
    """
    SiliconFlow 重排序器
    ...
    def __init__(self,
        ...
        if not self.api_key:
            print("Warning: SILICONFLOW_API_KEY not set. SiliconFlow reranker will not work.")
    ...
    def rerank(self,
        ...
        try:
            ...
            print(f"SiliconFlow rerank failed: {e}")
            # 失败时返回原始顺序
            return [
                {'content': doc, 'score': 0.5, 'index': i}
                for i, doc in enumerate(documents)
    ...
    def rerank_with_original(self,
        ...
        if not self.api_key:
            print("Warning: SILICONFLOW_API_KEY not set, skipping rerank")
